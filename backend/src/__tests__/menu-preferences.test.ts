...
async function registerUser(
  ...
  firstName: string,
  lastName: string,
  client: ReturnType<typeof request> | ReturnType<typeof request.agent> = request(app)
) {
...
describe("Menu preferences", () => {
  it("stores only the three most recent profile visits per user", async () => {
    ...
    const owner = await registerUser("menu-owner@example.com", "Menu", "Owner", ownerAgent);
    ...
    const fourthVisit = await ownerAgent
      .post(`/api/v1/users/${owner.userId}/recent-visits`)
    ...
    const revisited = await ownerAgent
      .post(`/api/v1/users/${owner.userId}/recent-visits`)
      .send({ profileId: profile2.userId })
      .expect(200);
  ...
  it("persists shortcuts in order and drops the oldest beyond four", async () => {
    const ownerAgent = request.agent(app);
    const owner = await registerUser("shortcut-owner@example.com", "Shortcut", "Owner", ownerAgent);
    ...
    const fifthShortcut = await ownerAgent
      .post(`/api/v1/users/${owner.userId}/shortcuts`)
